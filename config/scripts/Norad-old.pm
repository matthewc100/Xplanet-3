...
sub get_noraddata {
    # Key Steps
        ...
        # File Existence Check: 
            # Uses -f to check if the TLE file exists. If it does, the stat function is used to retrieve file metadata, such as the 
            # file's last modified time and size.
        # Check File Freshness: 
            # The age of the file is compared to the maximum allowed download frequency (12 hours in this case). If the file is younger 
            # than that threshold, it’s 
            # considered up to date, and the flag is set to 3.
        # Download or Check NORAD:
            # If the file is not up to date, the subroutine norad_checked() is called, which is likely responsible for either downloading 
            # new data or verifying its 
            # freshness from NORAD.
        # Return Value:
            # The subroutine returns a flag indicating the status of the TLE data retrieval. Depending on the situation, it could 
            # return 1 (initial value), 3 (if up to date), or another value from norad_checked().
...
sub norad_checked {
    ...
    if ($isstxt eq 'FAILED') {
        ...
        # this line needs to call Globals::file_header, to update the header of the marker file
        Label::file_header($iss_file, $mf);
...
sub update_file {
    ...
    if ($type eq 'ISS') {
        $noradsettings->{'NoradIssImage'} = 'iss_transparent.png';  # Set ISS image name.
    } elsif ($type eq 'HST') {
        $noradsettings->{'NoradHstImage'} = 'hst_transparent.png';  # Set HST image name.
    } elsif ($type eq 'STS') {
